...
namespace O5M.Helper
{
	public static class VarintBitConverter
	{
		...
		/// </summary>
		/// <param name="bytes">Varint encoded array of bytes.</param>
		/// <param name="readBytes">read bytes.</param>
		/// <returns>16-bit signed value</returns>
		public static short ToInt16(byte[] bytes, out int readBytes)
		{
			var zigzag = ToTarget(bytes, 16, out readBytes);
			return (short)DecodeZigZag(zigzag);
		}
		...
		/// <param name="readBytes">read bytes.</param>
		/// <returns>16-bit usigned value</returns>
		public static ushort ToUInt16(byte[] bytes, out int readBytes)
		{
			return (ushort)ToTarget(bytes, 16, out readBytes);
		}
		...
		/// <param name="readBytes">read bytes.</param>
		/// <returns>32-bit signed value</returns>
		public static int ToInt32(byte[] bytes, out int readBytes)
		{
			var zigzag = ToTarget(bytes, 32, out readBytes);
			return (int)DecodeZigZag(zigzag);
		}
		...
		public static uint ToUInt32(byte[] bytes, out int readBytes)
		{
			return (uint)ToTarget(bytes, 32, out readBytes);
		}
		...
		/// <param name="readBytes">read bytes.</param>
		/// <returns>64-bit signed value</returns>
		public static long ToInt64(byte[] bytes, out int readBytes)
		{
			var zigzag = ToTarget(bytes, 64, out readBytes);
			return DecodeZigZag(zigzag);
		}
		...
		/// <param name="readBytes">read bytes.</param>
		/// <returns>64-bit unsigned value</returns>
		public static ulong ToUInt64(byte[] bytes, out int readBytes)
		{
			return ToTarget(bytes, 64, out readBytes);
		}
		...
		private static ulong ToTarget(byte[] bytes, int sizeBytes)
		{
			int readBytes;
			return ToTarget(bytes, sizeBytes, out readBytes);
		}

		private static ulong ToTarget(byte[] bytes, int sizeBytes, out int readBytes)
		{
			int shift = 0;
			ulong result = 0;
			readBytes = 0;
